...
const eventTypeConfig: Record<MockEvent['type'], { icon: typeof Terminal; label: string; badgeClass: string }> = {
  tool_call: { icon: Terminal, label: 'Tool', badgeClass: 'bg-[var(--aiox-blue)]/15 text-[var(--aiox-blue)]' },
  message: { icon: MessageSquare, label: 'Message', badgeClass: 'bg-[var(--aiox-blue)]/15 text-[var(--aiox-blue)]' },
  error: { icon: AlertTriangle, label: 'Error', badgeClass: 'bg-[var(--bb-error)]/15 text-[var(--bb-error)]' },
  system: { icon: Settings2, label: 'System', badgeClass: 'bg-[var(--aiox-gray-muted)]/15 text-[var(--aiox-gray-muted)]' },
};
...
function EventRow({ event }: { event: MockEvent }) {
  ...
  return (
    <div
      className={cn('flex items-start gap-3 px-3 py-2 rounded-lg hover:bg-white/5 transition-colors', event.type === 'error' && 'bg-[var(--bb-error)]/5')}
    >
      ...
      {event.success !== undefined && (event.success ? <CheckCircle2 className="h-3.5 w-3.5 text-[var(--color-status-success)] flex-shrink-0" /> : <AlertCircle className="h-3.5 w-3.5 text-[var(--bb-error)] flex-shrink-0" />)}
    </div>
  );
}
    ...
    <>
      ...
      <div className="flex items-center gap-2 mb-3 flex-shrink-0">
        {filterTypes.map((type) => {
          ...
          return (
            <CockpitButton key={type} size="sm" variant="ghost" className={cn('text-xs', !isActive && 'opacity-40')} leftIcon={<Icon className="h-3 w-3" />} onClick={() => onToggleFilter(type)}>
              {config.label}
            </CockpitButton>
          );
        })}
