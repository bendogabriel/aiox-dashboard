                ...
                {isLoading ? (
                  ...
                  <>
                    ...
                    {activeTab === 'prompts' && (
                      <div className="space-y-2">
                        {agentPrompts.length === 0 ? (
                          <EmptyState message="Nenhum prompt sugerido para este agent" />
                        ) : (
                    ...
                    {activeTab === 'tasks' && (
                      <div className="space-y-2">
                        {tasks.length === 0 ? (
                          <EmptyState message="Nenhuma task definida para este squad" />
                        ) : (
                          tasks.map((task) => (
                            <CommandItem
                              key={task.id}
                    ...
                    {activeTab === 'workflows' && (
                      <div className="space-y-2">
                        {workflows.length === 0 ? (
                          <EmptyState message="Nenhum workflow definido para este squad" />
                        ) : (
...
const agentSuggestions: Record<string, AgentSuggestion[]> = {
  // Copywriting Squad
  'gary-halbert': [
    { icon: '✉️', label: 'Carta de vendas', prompt: 'Escreva uma carta de vendas estilo Gary Halbert para um produto digital de produtividade' },
    { icon: '🎯', label: 'Bullets que vendem', prompt: 'Crie 10 fascination bullets para uma página de vendas de curso online' },
    { icon: '📧', label: 'Email de vendas', prompt: 'Escreva um email de vendas direto e pessoal para reativar clientes inativos' },
  ],
  'eugene-schwartz': [
    { icon: '🧠', label: 'Níveis de consciência', prompt: 'Analise o nível de consciência do meu público e sugira a melhor abordagem de copy' },
    { icon: '📝', label: 'Headline breakthrough', prompt: 'Crie 5 headlines usando a técnica de mass desire do Eugene Schwartz' },
    { icon: '🔥', label: 'Lead magnético', prompt: 'Escreva um lead que capture atenção imediata para uma oferta de consultoria' },
  ],
  'david-ogilvy': [
    { icon: '📰', label: 'Anúncio clássico', prompt: 'Crie um anúncio no estilo Ogilvy para uma marca de luxo' },
    { icon: '🎨', label: 'Brand copy', prompt: 'Desenvolva o tom de voz e messaging para uma nova marca premium' },
    { icon: '📊', label: 'Copy com dados', prompt: 'Escreva um texto de vendas baseado em pesquisa e dados concretos' },
  ],
  // YouTube Content Squad
  'roteirista': [
    { icon: '🎬', label: 'Roteiro completo', prompt: 'Crie um roteiro de 10 minutos sobre [tema] com ganchos de retenção' },
    { icon: '🪝', label: 'Ganchos de abertura', prompt: 'Sugira 5 ganchos de abertura para um vídeo sobre produtividade' },
    { icon: '📋', label: 'Estrutura de vídeo', prompt: 'Monte a estrutura ideal para um vídeo educacional de YouTube' },
  ],
  'title-writer': [
    { icon: '🏷️', label: 'Títulos virais', prompt: 'Crie 10 títulos otimizados para CTR sobre [tema do vídeo]' },
    { icon: '🔍', label: 'Análise de título', prompt: 'Analise este título e sugira melhorias para aumentar CTR' },
    { icon: '⚡', label: 'Títulos A/B', prompt: 'Gere variações de título para teste A/B no YouTube' },
  ],
  'thumbnail-strategist': [
    { icon: '🖼️', label: 'Conceito visual', prompt: 'Defina o conceito visual e elementos-chave para thumbnail de vídeo sobre [tema]' },
    { icon: '🎯', label: 'Estratégia de thumb', prompt: 'Analise thumbnails de concorrentes e sugira estratégia diferenciada' },
    { icon: '👁️', label: 'Elementos de atenção', prompt: 'Quais elementos visuais usar para destacar na home do YouTube?' },
  ],
  'deep-researcher': [
    { icon: '🔬', label: 'Pesquisa profunda', prompt: 'Faça uma pesquisa aprofundada sobre [tema] com fontes acadêmicas' },
    { icon: '📚', label: 'Estado da arte', prompt: 'Qual o estado atual da pesquisa científica sobre [tema]?' },
    { icon: '📊', label: 'Dados e estatísticas', prompt: 'Encontre dados e estatísticas relevantes sobre [tema] para fundamentar meu conteúdo' },
  ],
  'briefing-creator': [
    { icon: '📋', label: 'Briefing completo', prompt: 'Crie um briefing estruturado para produção de vídeo sobre [tema]' },
    { icon: '🎯', label: 'Direcionamento criativo', prompt: 'Sintetize esta pesquisa em direcionamentos claros para o roteirista' },
    { icon: '✅', label: 'Checklist de briefing', prompt: 'Monte um checklist de todos os elementos necessários para o briefing' },
  ],
  // Design System Squad
  'brad-frost': [
    { icon: '🧱', label: 'Design System', prompt: 'Estruture um design system com atomic design para minha aplicação' },
    { icon: '📦', label: 'Componentes', prompt: 'Defina a hierarquia de componentes (átomos, moléculas, organismos) para um dashboard' },
    { icon: '📖', label: 'Documentação', prompt: 'Crie a documentação de um componente de botão seguindo best practices' },
  ],
  'dan-mall': [
    { icon: '🎨', label: 'Design tokens', prompt: 'Defina os design tokens essenciais para consistência visual' },
    { icon: '🔄', label: 'Design handoff', prompt: 'Como estruturar um handoff eficiente entre design e desenvolvimento?' },
    { icon: '📐', label: 'Grid system', prompt: 'Sugira um grid system responsivo para uma aplicação web moderna' },
  ],
  // Orchestrator Squad
  'supervisor-sistema': [
    { icon: '🎯', label: 'Coordenar squads', prompt: 'Coordene uma tarefa complexa envolvendo múltiplos squads' },
    { icon: '📊', label: 'Análise de gaps', prompt: 'Identifique gaps de cobertura nos squads atuais' },
    { icon: '🔄', label: 'Otimizar workflow', prompt: 'Sugira melhorias no workflow entre os squads de conteúdo' },
  ],
  'roteador': [
    { icon: '🔀', label: 'Rotear demanda', prompt: 'Qual o melhor squad e agente para criar uma campanha de lançamento?' },
    { icon: '🎯', label: 'Classificar tarefa', prompt: 'Classifique esta tarefa e indique o fluxo ideal de execução' },
    { icon: '📋', label: 'Plano de ação', prompt: 'Monte um plano de ação com os agentes necessários para [objetivo]' },
  ],
};
...
const squadSuggestions: Record<string, AgentSuggestion[]> = {
  copywriting: [
    { icon: '✍️', label: 'Copy de vendas', prompt: 'Escreva um texto de vendas persuasivo para [produto/serviço]' },
    { icon: '📧', label: 'Sequência de emails', prompt: 'Crie uma sequência de 5 emails para lançamento de produto' },
    { icon: '🎯', label: 'Headlines', prompt: 'Gere 10 headlines impactantes para [tema]' },
  ],
  design: [
    { icon: '🎨', label: 'Sistema visual', prompt: 'Defina um sistema visual completo para minha aplicação' },
    { icon: '📱', label: 'UI patterns', prompt: 'Quais UI patterns usar para uma experiência mobile-first?' },
    { icon: '🧩', label: 'Componentes', prompt: 'Estruture uma biblioteca de componentes reutilizáveis' },
  ],
  creator: [
    { icon: '🎬', label: 'Conteúdo', prompt: 'Crie um plano de conteúdo para [plataforma] sobre [tema]' },
    { icon: '📅', label: 'Calendário', prompt: 'Monte um calendário editorial para o próximo mês' },
    { icon: '💡', label: 'Ideias', prompt: 'Sugira 20 ideias de conteúdo para engajar minha audiência' },
  ],
  orchestrator: [
    { icon: '🎯', label: 'Orquestrar', prompt: 'Coordene uma entrega complexa entre múltiplos especialistas' },
    { icon: '📊', label: 'Analisar', prompt: 'Analise o fluxo atual e identifique oportunidades de melhoria' },
    { icon: '🔄', label: 'Workflow', prompt: 'Desenhe um workflow otimizado para [processo]' },
  ],
};
...
function getCommandIcon(command: string): string {
  ...
  if (cmd.includes('letter') || cmd.includes('carta')) return '✉️';
  if (cmd.includes('email')) return '📧';
  if (cmd.includes('headline') || cmd.includes('titulo')) return '🏷️';
  if (cmd.includes('bullet')) return '🎯';
  if (cmd.includes('roteiro') || cmd.includes('script')) return '🎬';
  if (cmd.includes('research') || cmd.includes('pesquisa')) return '🔬';
  if (cmd.includes('briefing')) return '📋';
  if (cmd.includes('thumbnail') || cmd.includes('thumb')) return '🖼️';
  if (cmd.includes('hook') || cmd.includes('gancho')) return '🪝';
  if (cmd.includes('copy')) return '✍️';
  if (cmd.includes('análise') || cmd.includes('analise') || cmd.includes('analysis')) return '📊';
  if (cmd.includes('sequence') || cmd.includes('sequência')) return '🔄';
  if (cmd.includes('lead')) return '🔥';
  if (cmd.includes('design')) return '🎨';
  if (cmd.includes('component')) return '🧱';
  return '⚡';
}
