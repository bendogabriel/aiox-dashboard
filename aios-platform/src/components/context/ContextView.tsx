...
function CollapsibleSection({ title, icon, count, defaultOpen = true, children }: CollapsibleSectionProps) {
  ...
  return (
    <CockpitCard padding="none" className="overflow-hidden">
      <button
            ...
            <div key={agent.id ?? agent.name} className="flex items-center justify-between glass-subtle rounded-lg px-3 py-2">
              <div className="flex items-center gap-2.5">
                <div className={cn(
                  ...
                  agent.model === 'opus' ? 'bg-[var(--aiox-gray-muted)]/20' : agent.model === 'haiku' ? 'bg-[var(--color-status-success)]/20' : 'bg-[var(--aiox-blue)]/20',
                )}>
                  <Bot size={12} className={cn(
                    agent.model === 'opus' ? 'text-[var(--aiox-gray-muted)]' : agent.model === 'haiku' ? 'text-[var(--color-status-success)]' : 'text-[var(--aiox-blue)]',
                  )} />
                </div>
                <div>
